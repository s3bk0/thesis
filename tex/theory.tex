...
Considering the case $\theta_1 > \theta_{tot}$, $\theta_2$ takes the shape of
...
From that we can define reflectivity $\mathcal{R}$ and transmittivity
$\mathcal{T}$ of the
...
To evaluate the transmission through the stack of layers, it is now the task to
calculate the coefficients $t_{N,i}$ and $r_{1,i}$ under given coefficients
$t_{1,i}$ and with the assumption $r_{N,i}=0$ for all $i\in\{L,TH,TV\}$. We
...
the following, two similar solution methods are introduced.
...
To introduce the matrix formalism it is useful to focus on a single interface
with index $n$ between layers $n-1$ and $n$ located at depth $z_n$. We can
define a generalised state vector $\uline{v}_i$  in each layer $i$ as
\cite{Fomenko2014}
\begin{equation}
...
\begin{equation} \label{eq:PmatTMM}
    \uuline{P_n}(z) = \mathit{diag}\{ e^{ik_{z,n}^{(L)}z},
    e^{ik_{z,n}^{(TH)}z},
    e^{ik_{z,n}^{(TV)}z}, e^{-ik_{z,n}^{(L)}z}, e^{-ik_{z,n}^{(TH)}z},
    e^{-ik_{z,n}^{(TV)}z}\}
\end{equation}
The addend $k_{y,n} y$ was removed from the expression as it does not change at
the boundary. This automatically sets the phase of the displacement wave to
zero at the upper boundary of the layer. The coefficients at the lower boundary
are in the following denoted by $\uline{a}_n^\prime = \uuline{P_n}(-d_n)\
    \uline{a}_n$.
...
\end{equation}
If we assume that $\uuline{M}_{n}$ is invertable, we can define the
Transfermatrix $S_n$ by
\begin{equation}
...
With this knowledge we can assemble a global transfermatrix for all interfaces
by
\begin{equation}
...
will be called from now on LSE-method (Linear System of Equations). Its
...
\begin{equation}
    ...
    \end{pmatrix}
\end{equation}
to be able to invert it. By setting $\uline{r}_N = 0$ as before we obtain
$\uline{r}_0$ and $\uline{t}_N$ after inverting the $16\cross 16$ matrix.